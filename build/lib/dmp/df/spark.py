...
class SparkPipeline(Pipeline):
    ...
    def connect_db(self, wf_connection_id="", connection_engine=None):
        """
    ...
    def read_sql(self, sql: any, log: bool = True,
                 replace_params: dict = {}, concat_params={}, spark_options={}, inplace=True):
        """
    ...
    def read_dict(self, data, schema=None):
        """
    ...
    def read_batch(self, data, checkpoint_func, batch_size=DEFAULT_BATCH_SIZE,
                   schema=None
                   ):
        """
        ...
        total = 0
        for batch in f:
            df_temp = self.extract_batch(batch, sp, schema)
    ...
    def read_csv(self, file_path, options_csv: dict = {}):
        ...
        for k, v in default_option.items():
            df = df.option(k, v)
        self._df_result = df.csv(file_path)
        return self
