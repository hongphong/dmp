...
class Pipeline():
    ...
    def __init__(self, df=None, attach=""):
        self._df_result = df
    ...
    @classmethod
    def connect_db(cls, wf_connection_id=""):
        """
        ...
        if not self._db_sql_engine:
            try:
                ...
                db = self._db_wf_connection.get_wf_db()
                try:
                    self._db_sql_engine = db.get_sqlalchemy_engine()
                except BaseException as e:
                    logger.info("Ignore db_sql_engine because class: [%s] not support get_sqlalchemy_engine" % type(
                        self.db).__name__)
                    self._db_sql_engine = None
            except BaseException as e:
